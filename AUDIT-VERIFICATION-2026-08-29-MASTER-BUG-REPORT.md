...
| 11 | Tool instance reused across requests | ⚠️ PARTIALLY TRUE | Confirmed: `capabilities/investigation.py::_make_tool_capability` creates `tool = tool_cls()` once and captures it in a closure reused by every request. However, checked all 5 `InvestigationTool` subclasses in `services/investigation/tools.py` — none define `__init__` or store any mutable instance state; all data flows through method parameters (`db`, `InvestigationContext`). The pattern is real but currently harmless. Worth fixing as a latent-bug/code-smell, not an active one. |
