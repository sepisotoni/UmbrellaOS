...
const DashboardContent: React.FC = () => {
  ...
  if (activeTab !== 'login' && currentUser && DENIED_ROLES.includes(currentUser.role)) {
    return (
      <div className="h-screen w-screen overflow-y-auto font-sans bg-[#02040a] text-slate-100">
        <AccessDeniedView />
      </div>
    );
  }
  ...
  return (
    <div className="h-screen max-h-screen w-screen flex flex-col overflow-hidden font-sans bg-[#02040a] text-slate-100 selection:bg-indigo-500/30 selection:text-indigo-200">
      ...
      <div className="flex-1 flex flex-row overflow-hidden min-h-0 w-full bg-[#02040a]">
        {/* Left Sidebar */}
        <Sidebar onOpenBanModal={() => handleOpenBanModalWithTarget('')} />

        {/* Dynamic Main Workspace Container */}
        <main className="flex-1 overflow-y-auto min-h-0 px-4 py-6 md:px-8 max-w-7xl mx-auto w-full bg-[#02040a]">
          {activeTab === 'overview' && (
            <OverviewView
