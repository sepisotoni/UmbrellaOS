...
def upgrade() -> None:
    ...
    # No explicit op.create_index for player_uuid here — same real,
    # DB-agnostic duplicate-index bug as 004_phase8_verification.py's
    # verification_codes table (see that file's comment): `index=True`
    # on the column above already creates
    # `ix_suspicion_events_player_uuid` implicitly via op.create_table.
...
def downgrade() -> None:
    ...
    op.drop_constraint(
        "fk_alt_group_members_group_id", "alt_group_members", type_="foreignkey"
    )
