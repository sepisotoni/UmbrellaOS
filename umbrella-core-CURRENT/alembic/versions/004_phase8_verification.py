...
def upgrade() -> None:
    ...
    # explicit op.create_index call for the same name used to sit here
    # and duplicate it — a real bug on any DB (not sqlite-specific; a
    # real Postgres run would raise the same "index already exists"
    # error), found by actually running `alembic upgrade head` against a
