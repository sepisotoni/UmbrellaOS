...
def test_capability_name_must_be_namespaced():
    ...
    with pytest.raises(ValueError):
        CapabilitySpec(
            name="notnamespaced", summary="x", params_model=EchoParams, handler=_handler
        )
...
@pytest.mark.asyncio
async def test_successful_call_returns_result_and_records_audit(
    ...
    async with db_session() as db:
        ctx = _make_ctx(db, is_superuser=True)
        result = await fresh_registry.call(
            "test.echo.audited", ctx, {"message": "hello"}
        )
        ...
        rows = (
            (
                await db.execute(
                    select(AuditLog).where(AuditLog.action == "test.echo.audited")
                )
            )
            .scalars()
            .all()
...
async def test_call_without_required_permission_is_denied_and_not_audited_as_success(
    ...
    async with db_session() as db:
        ...
        rows = (
            (
                await db.execute(
                    select(AuditLog).where(AuditLog.action == "test.echo.guarded")
                )
            )
            .scalars()
            .all()
...
@pytest.mark.asyncio
async def test_call_with_required_permission_present_succeeds(
    fresh_registry, db_session
):
    from registry.spec import CapabilitySpec
...
@pytest.mark.asyncio
async def test_handler_exception_is_audited_as_error_then_reraised(
    fresh_registry, db_session
):
    from registry.spec import CapabilitySpec
    ...
    async with db_session() as db:
        ...
        rows = (
            (
                await db.execute(
                    select(AuditLog).where(AuditLog.action == "test.echo.failing")
                )
            )
            .scalars()
            .all()
...
async def test_superuser_bypasses_any_required_permission(fresh_registry, db_session):
    ...
    async with db_session() as db:
        ctx = _make_ctx(db, is_superuser=True)
        result = await fresh_registry.call(
            "test.echo.superuser_only", ctx, {"message": "ok"}
        )
        assert result.echoed == "ok"
...
def test_unregister_removes_capability(fresh_registry):
    ...
    fresh_registry.register(
        CapabilitySpec(
            name="test.remove.me", summary="x", params_model=EchoParams, handler=_h
        )
    )
...
def test_unregister_then_reregister_same_name_succeeds(fresh_registry):
    ...
    spec_v1 = CapabilitySpec(
        name="plugin.demo.thing", summary="v1", params_model=EchoParams, handler=_h
    )
    fresh_registry.register(spec_v1)
    fresh_registry.unregister("plugin.demo.thing")

    spec_v2 = CapabilitySpec(
        name="plugin.demo.thing", summary="v2", params_model=EchoParams, handler=_h
    )
...
def test_list_returns_sorted_specs(fresh_registry):
    ...
    fresh_registry.register(
        CapabilitySpec(
            name="zzz.last", summary="z", params_model=EchoParams, handler=_h
        )
    )
    fresh_registry.register(
        CapabilitySpec(
            name="aaa.first", summary="a", params_model=EchoParams, handler=_h
        )
    )
