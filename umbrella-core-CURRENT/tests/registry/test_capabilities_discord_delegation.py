...
@pytest.mark.asyncio
async def test_delegated_call_for_unlinked_discord_user_gets_only_base_permissions(
    client,
):
    plaintext = await _create_delegate_key(
        client, extra_permissions=["hosting.server.view"]
    )
...
@pytest.mark.asyncio
async def test_delegated_call_for_linked_staff_user_gets_unioned_permissions(
    client, db_session
):
    async with db_session() as db:
        ...
        db.add(role)
        await db.flush()
        perm = await _get_seeded_permission(
            db, "moderation_intelligence.escalation.manage"
        )
...
@pytest.mark.asyncio
async def test_delegated_call_for_deactivated_linked_user_falls_back_to_base_only(
    client, db_session
):
    async with db_session() as db:
...
@pytest.mark.asyncio
async def test_header_ignored_without_delegate_permission_falls_back_to_key_scope(
    client,
):
    ...
    plaintext = await _create_non_delegate_key(
        client, permissions=["hosting.server.view"]
    )
...
@pytest.mark.asyncio
async def test_source_is_discord_when_header_present_even_without_delegate_permission(
    client,
):
...
@pytest.mark.asyncio
async def test_delegated_call_is_denied_for_capability_neither_scope_grants(
    client, db_session
):
    async with db_session() as db:
