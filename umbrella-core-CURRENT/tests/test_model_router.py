...
class FakeProvider:
    ...
    async def generate(
        self, model, system_prompt, user_prompt, max_tokens=1024, temperature=0.7
    ):
        self.call_count += 1
        if self._should_fail:
            raise ProviderError(f"simulated failure from {self.name}")
        return GenerationResult(
            text=f"response from {self.name}/{model}", model_name=model, latency_ms=42
        )


async def _enable_and_key(
    db, provider: str, enabled: bool = True, key: str = "sk-test"
):
    enabled_setting = f"ai.{provider}_enabled"
    await SettingsService.update(
        db, enabled_setting, "true" if enabled else "false", actor="test"
    )
    key_setting = {
...
async def _add_config(
    db, provider: str, model_name: str, task_type: str, priority: int = 100
) -> AIModelConfig:
    config = AIModelConfig(
        provider=provider, model_name=model_name, task_type=task_type, priority=priority
    )
    db.add(config)
    await db.flush()
    return config
...
async def test_no_candidates_configured_raises_clear_error(db_session):
    async with db_session() as db:
        with pytest.raises(
            NoAvailableModelError, match="no eligible candidates configured"
        ):
...
async def test_selects_highest_priority_candidate_first(db_session, monkeypatch):
    async with db_session() as db:
        ...
        async def fake_build(db, provider_name):
            return {"anthropic": fake_anthropic, "openrouter": fake_openrouter}[
                provider_name
            ]
...
@pytest.mark.asyncio
async def test_marks_unhealthy_after_threshold_consecutive_failures(
...
@pytest.mark.asyncio
async def test_unhealthy_candidate_retried_after_cooldown_elapses(
    db_session, monkeypatch
):
    settings = get_settings()
    ...
    async with db_session() as db:
        ...
        config = await _add_config(db, "anthropic", "claude-x", "chat", priority=10)
        config.is_healthy = False
        config.last_failure_at = datetime.now(timezone.utc) - timedelta(
            seconds=120
        )  # long past cooldown
        await db.commit()
...
@pytest.mark.asyncio
async def test_successful_call_resets_failure_count_and_marks_healthy(
    db_session, monkeypatch
):
    async with db_session() as db:
        await _enable_and_key(db, "anthropic")
...
@pytest.mark.asyncio
async def test_disabled_provider_is_skipped_even_with_a_configured_model_row(
    db_session, monkeypatch
):
    async with db_session() as db:
...
async def test_exclude_providers_skips_named_provider(db_session, monkeypatch):
    async with db_session() as db:
        ...
        routed = await ModelRouter.generate(
            db, "chat", "sys", "user", exclude_providers={"anthropic"}
        )
        assert routed.provider == "openrouter"
