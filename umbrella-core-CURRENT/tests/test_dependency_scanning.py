...
def test_main_generates_valid_cyclonedx_json_against_real_requirements(tmp_path):
    ...
    exit_code = generate_sbom.main(
        ["--requirements", str(real_requirements), "--output", str(output)]
    )
    assert exit_code == 0
...
def test_severity_rank_orders_low_to_critical():
    assert scan_dependencies._severity_rank(
        {"severity": "low"}
    ) < scan_dependencies._severity_rank({"severity": "high"})
    assert scan_dependencies._severity_rank(
        {"severity": "high"}
    ) < scan_dependencies._severity_rank({"severity": "critical"})


def test_severity_rank_treats_missing_severity_as_unknown_worst_case():
    assert scan_dependencies._severity_rank(
        {}
    ) == scan_dependencies._SEVERITY_ORDER.index("unknown")
...
def test_main_reports_no_findings_when_report_has_no_vulns(monkeypatch):
    monkeypatch.setattr(
        ...
        "run_pip_audit",
        lambda req: {
            "dependencies": [{"name": "fastapi", "version": "0.115.5", "vulns": []}]
        },
    )
        ...
        "dependencies": [
            {
                "name": "badpkg",
                "version": "1.0.0",
                "vulns": [{"id": "CVE-2024-0001", "severity": "critical"}],
            }
        ]
    }
        ...
        "dependencies": [
            {
                "name": "mildpkg",
                "version": "1.0.0",
                "vulns": [{"id": "CVE-2024-0002", "severity": "low"}],
            }
        ]
    }
    monkeypatch.setattr(scan_dependencies, "run_pip_audit", lambda req: fake_report)
...
def test_run_pip_audit_raises_on_empty_stdout_with_nonzero_exit(monkeypatch):
    ...
    monkeypatch.setattr(
        scan_dependencies.subprocess, "run", lambda *a, **k: _FakeCompletedProcess()
    )
