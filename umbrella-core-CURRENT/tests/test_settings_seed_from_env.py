...
@pytest.mark.asyncio
async def test_seed_defaults_does_not_sync_from_env_when_flag_is_false(
...
@pytest.mark.asyncio
async def test_seed_defaults_gap_fills_from_env_when_flag_is_true(
    db_session, temp_env_file, monkeypatch
):
    settings = get_settings()
...
async def test_seed_defaults_never_overwrites_existing_dashboard_value_in_gap_fill_mode(
    ...
    async with db_session() as db:
        ...
        await SettingsService.update(
            db, "ai.openrouter_key", "sk-set-via-dashboard", actor="test"
        )

    async with db_session() as db:
        await SettingsService.seed_defaults(
            db
        )  # a second "boot" with the flag still true
        row = await db.scalar(select(Setting).where(Setting.key == "ai.openrouter_key"))
...
@pytest.mark.asyncio
async def test_seed_defaults_auto_resets_seed_from_env_flag_in_env_file(
    db_session, temp_env_file, monkeypatch
):
    settings = get_settings()
    monkeypatch.setattr(settings, "seed_from_env", True)
...
@pytest.mark.asyncio
async def test_seed_defaults_force_override_also_resets_itself(
    db_session, temp_env_file, monkeypatch
):
    settings = get_settings()
    monkeypatch.setattr(settings, "seed_from_env", True)
    ...
    assert "SEED_FROM_ENV=false" in content or "SEED_FROM_ENV='false'" in content
    assert (
        "FORCE_ENV_OVERRIDE=false" in content or "FORCE_ENV_OVERRIDE='false'" in content
    )
