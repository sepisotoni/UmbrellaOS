...
from services.knowledge.service import (
    KnowledgeService,
    channel_names_configured,
    is_knowledge_channel,
)
...
async def test_channel_names_configured_parses_comma_separated_list(monkeypatch):
    monkeypatch.setattr(
        get_settings(), "knowledge_channel_names", "ai-faq, ai-news ,AI-STORE"
    )
...
async def test_index_entry_indexes_configured_channel(db_session, monkeypatch):
    ...
    async with db_session() as db:
        entry = await KnowledgeService.index_entry(
            ...
            author_id="user-1",
            author_name="Alice",
            content="The server IP is play.example.com",
        )
...
@pytest.mark.asyncio
async def test_search_only_returns_approved_non_superseded_entries(
    ...
    async with db_session() as db:
        await KnowledgeService.index_entry(
            ...
            author_id="user-1",
            author_name="Alice",
            content="the store link is example.com/store",
        )
        await db.commit()
...
@pytest.mark.asyncio
async def test_propose_and_approve_correction_supersedes_original(
    db_session, monkeypatch
):
    monkeypatch.setattr(get_settings(), "knowledge_channel_names", "ai-faq")
    async with db_session() as db:
        original = await KnowledgeService.index_entry(
            ...
            channel_id="chan-1",
            channel_name="ai-faq",
            discord_message_id="msg-6",
            author_id="user-1",
            author_name="Alice",
            content="old IP: 1.2.3.4",
        )
        await db.commit()
        ...
        correction = await KnowledgeService.propose_correction(
            ...
            channel_id="chan-1",
            channel_name="ai-faq",
            discord_message_id="msg-7",
            author_id="staff-1",
            author_name="Staff",
            content="new IP: 5.6.7.8",
        )
        await db.commit()
        ...
        approved = await KnowledgeService.approve(
            db, correction.id, reviewed_by="admin-1"
        )
        await db.commit()
        assert approved.review_status == KnowledgeReviewStatus.APPROVED

        results_after = await KnowledgeService.search(db, "IP")
        assert {r.id for r in results_after} == {
            correction.id
        }  # original is now superseded, excluded
...
async def test_reject_correction_leaves_original_live(db_session, monkeypatch):
    ...
    async with db_session() as db:
        original = await KnowledgeService.index_entry(
            ...
            channel_id="chan-1",
            channel_name="ai-faq",
            discord_message_id="msg-8",
            author_id="user-1",
            author_name="Alice",
            content="store hours: 9-5",
        )
        await db.commit()
        ...
        correction = await KnowledgeService.propose_correction(
            ...
            author_id="staff-1",
            author_name="Staff",
            content="store hours: 24/7",
        )
        await db.commit()

        rejected = await KnowledgeService.reject(
            db, correction.id, reviewed_by="admin-1"
        )
        await db.commit()
...
async def test_list_pending_only_shows_pending(db_session, monkeypatch):
    ...
    async with db_session() as db:
        ...
        await KnowledgeService.propose_correction(
            ...
            channel_id="chan-1",
            channel_name="ai-faq",
            discord_message_id="msg-11",
            author_id="staff-1",
            author_name="Staff",
            content="entry A corrected",
        )
        await db.commit()
