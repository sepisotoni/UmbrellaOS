    ...
    async with db_session() as db:
        ...
        db.add(
            ServerMetricSnapshot(
                server_id="srv-3", tps=20.0, online_count=1, recorded_at=old
            )
        )
        db.add(
            ServerMetricSnapshot(
                server_id="srv-3", tps=18.0, online_count=2, recorded_at=recent
            )
        )
        await db.flush()
...
async def test_recent_snapshots_orders_oldest_first(db_session):
    async with db_session() as db:
        ...
        db.add(
            ServerMetricSnapshot(
                server_id="srv-4", tps=15.0, online_count=1, recorded_at=t2
            )
        )
        db.add(
            ServerMetricSnapshot(
                server_id="srv-4", tps=20.0, online_count=1, recorded_at=t1
            )
        )
        await db.flush()

        snaps = await recent_snapshots(
            db, "srv-4", since=dt.datetime.now(dt.timezone.utc) - dt.timedelta(hours=1)
        )
...
async def test_purge_old_snapshots_removes_only_expired(db_session, monkeypatch):
    ...
    async with db_session() as db:
        ...
        db.add(
            ServerMetricSnapshot(
                server_id="srv-5", tps=20.0, online_count=1, recorded_at=old
            )
        )
        db.add(
            ServerMetricSnapshot(
                server_id="srv-5", tps=18.0, online_count=1, recorded_at=recent
            )
        )
        await db.flush()
        ...
        remaining = await recent_snapshots(
            db, "srv-5", since=dt.datetime.now(dt.timezone.utc) - dt.timedelta(hours=3)
        )
