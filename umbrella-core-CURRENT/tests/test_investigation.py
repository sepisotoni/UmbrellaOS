...
async def test_whitelist_status_tool_reports_no_target(db_session):
    async with db_session() as db:
        finding = await WhitelistStatusTool().run(
            db, InvestigationContext(target_user_id=None)
        )
...
async def test_whitelist_status_tool_reports_no_record(db_session):
    async with db_session() as db:
        finding = await WhitelistStatusTool().run(
            db, InvestigationContext(target_user_id="user-nowhere")
        )
...
async def test_whitelist_status_tool_reports_status(db_session):
    async with db_session() as db:
        ...
        finding = await WhitelistStatusTool().run(
            db, InvestigationContext(target_user_id="user-1")
        )
...
async def test_known_issues_tool_reports_open_issues_only(db_session):
    async with db_session() as db:
        ...
        finding = await KnownIssuesTool().run(
            db, InvestigationContext(target_user_id=None)
        )
...
async def test_maintenance_status_tool_matches_maintenance_terms(db_session):
    async with db_session() as db:
        ...
        db.add(
            KnownIssue(
                title="Chat bug",
                description="Emoji rendering wrong",
                is_resolved=False,
                created_by="staff-1",
            )
        )
        ...
        finding = await MaintenanceStatusTool().run(
            db, InvestigationContext(target_user_id=None)
        )
...
async def test_punishment_history_tool_reports_recent_actions(db_session):
    async with db_session() as db:
        ...
        finding = await PunishmentHistoryTool().run(
            db, InvestigationContext(target_user_id="user-2")
        )
...
async def test_linked_account_tool_uses_existing_discord_account_table(db_session):
    ...
    async with db_session() as db:
        ...
        finding = await LinkedAccountTool().run(
            db, InvestigationContext(target_user_id="user-3")
        )
...
async def test_linked_account_tool_ignores_unverified_links(db_session):
    async with db_session() as db:
        db.add(
            Player(uuid="22222222-2222-2222-2222-222222222222", username="Unverified")
        )
        db.add(
            DiscordAccount(
                discord_id="user-4",
        ...
        finding = await LinkedAccountTool().run(
            db, InvestigationContext(target_user_id="user-4")
        )
        assert "not linked" in finding.finding_text


@pytest.mark.asyncio
async def test_recent_announcements_tool_searches_knowledge_base(
    ...
    async with db_session() as db:
        await KnowledgeService.index_entry(
            ...
            channel_id="chan-1",
            channel_name="ai-faq",
            discord_message_id="msg-ra1",
            author_id="user-1",
            author_name="Alice",
            content="The store restocks every Friday",
        )
        await db.flush()
...
async def test_run_investigation_aggregates_every_tool(db_session):
    async with db_session() as db:
        result = await run_investigation(
            ...
            question="Why can't they join?",
        )
        await db.commit()

        assert (
            len(result["findings"]) == 6
        )  # every ported tool ran, including recent_announcements now
...
async def test_run_investigation_persists_one_finding_per_tool(db_session):
    async with db_session() as db:
        result = await run_investigation(
            db, requested_by="staff-1", target_user_id=None, question="test"
        )
        await db.commit()
