...
async def test_seed_defaults_is_idempotent(db_session):
    ...
    async with db_session() as db:
        ...
        assert len(titles) == len(
            set(titles)
        )  # no duplicates from running seeding twice
...
async def test_add_custom_rule(db_session):
    async with db_session() as db:
        rule = await ConstitutionService.add_rule(
            db,
            ConstitutionTier.SERVER,
            "Be concise in this Discord channel",
...
async def test_set_enabled_toggles_a_rule(db_session):
    async with db_session() as db:
        rule = await ConstitutionService.add_rule(
            db, ConstitutionTier.TASK, "Custom", "text"
        )
        await db.commit()
        rule_id = rule.id
...
async def test_build_system_prompt_includes_enabled_rules_in_tier_order(db_session):
    async with db_session() as db:
        await ConstitutionService.add_rule(
            db, ConstitutionTier.TASK, "Task rule", "task text"
        )
        await ConstitutionService.add_rule(
            db, ConstitutionTier.PLATFORM_SAFETY, "Safety rule", "safety text"
        )
        await db.commit()

        prompt = await ConstitutionService.build_system_prompt(
            db, "Summarize this incident."
        )
...
async def test_build_system_prompt_excludes_disabled_rules(db_session):
    async with db_session() as db:
        rule = await ConstitutionService.add_rule(
            db, ConstitutionTier.TASK, "Disabled rule", "should not appear"
        )
...
@pytest.mark.asyncio
async def test_build_system_prompt_with_no_rules_returns_just_the_task_prompt(
    db_session,
):
    async with db_session() as db:
...
async def test_set_enabled_raises_for_unknown_rule(db_session):
    async with db_session() as db:
        with pytest.raises(ConstitutionError):
            await ConstitutionService.set_enabled(
                db, "00000000-0000-0000-0000-000000000000", False
            )
