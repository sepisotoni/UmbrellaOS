...
async def test_record_writes_a_security_event_row(_patched_session, db_session):
    await threat_detection_service.record(
        event_type="auth_failure", source_ip="1.2.3.4"
    )
...
async def test_no_alert_below_threshold(_patched_session):
    alerted = await threat_detection_service.record(
        event_type="auth_failure", source_ip="1.2.3.4"
    )
...
@pytest.mark.asyncio
async def test_alert_publishes_security_threat_detected_event(
    ...
    async with db_session() as db:
        ...
        events = (
            (
                await db.execute(
                    select(Event).where(Event.topic == "security.threat_detected")
                )
            )
            .scalars()
            .all()
...
async def test_different_source_ips_tracked_independently(_patched_session):
    ...
    alerted = await threat_detection_service.record(
        event_type="auth_failure", source_ip="2.2.2.2"
    )
    assert alerted is False
...
async def test_sandbox_violation_alerts_on_first_occurrence(_patched_session):
    alerted = await threat_detection_service.record(
        event_type="sandbox_violation", identifier="some-plugin"
    )
    assert alerted is True
...
async def test_record_never_raises_when_db_unavailable(monkeypatch):
    ...
    monkeypatch.setattr(
        threat_detection_service, "AsyncSessionLocal", _BrokenSessionLocal()
    )
    result = await threat_detection_service.record(
        event_type="auth_failure", source_ip="1.2.3.4"
    )
