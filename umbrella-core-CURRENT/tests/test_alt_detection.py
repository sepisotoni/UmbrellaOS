...
async def test_post_alts_check_returns_score_and_triggers(client):
    ...
    response = await client.post(
        "/api/v1/alts/check", json=payload, headers=ADMIN_HEADERS
    )
...
async def test_post_alts_check_with_same_ip_increases_score(client):
    ...
    response = await client.post(
        "/api/v1/alts/check", json=payload2, headers=ADMIN_HEADERS
    )
    assert response.status_code == 200
    data = response.json()
...
async def test_post_alts_check_without_admin_key_returns_401(client):
    ...
    response = await client.post(
        "/api/v1/alts/check", json=payload, headers=WRONG_HEADERS
    )
...
async def test_get_alts_player_uuid_returns_suspicion_history(client):
    ...
    response = await client.get(
        "/api/v1/alts/player/ffffffff-aaaa-bbbb-cccc-dddddddddddd",
...
async def test_post_alts_false_positive_reduces_score(client):
    ...
    response = await client.get(
        "/api/v1/alts/player/aaaaaaaa-bbbb-cccc-dddd-000000000000",
        headers=ADMIN_HEADERS,
    )
    ...
    response = await client.post(
        "/api/v1/alts/false-positive", json=payload, headers=ADMIN_HEADERS
    )
    assert response.status_code == 200
...
async def test_post_alts_group_creates_alt_group(client):
    ...
    response = await client.post(
        "/api/v1/alts/group", json=payload, headers=ADMIN_HEADERS
    )
    assert response.status_code == 200
    data = response.json()
    assert "id" in data
