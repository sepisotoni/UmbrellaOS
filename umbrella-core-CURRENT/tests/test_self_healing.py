...
class FakeHealingDaemonClient:
    ...
    async def state(self, server_id):
        ...
        return ContainerState(
            ...
            started_at=None,
            finished_at=None,
            exit_code=1 if status == "crashed" else 0,
            oom_killed=False,
        )
...
async def test_repeated_crashes_escalate_to_suspension(db_session):
    ...
    for i in range(3):
        ...
        async with db_session() as db:
            ...
            assert crash_detected is True
            assert server.is_suspended is False, (
                f"should not be suspended after {i + 1} crashes"
            )
    ...
    assert (
        fake_client.start_calls == calls_before
    )  # no further restart attempted once suspended
...
async def test_manual_start_resets_crash_count(db_session):
    ...
    async with db_session() as db:
        server, _ = await ServerService.reconcile_server(
            db, server_id, daemon_client=fake_client
        )
        await db.commit()
        assert server.crash_count == 1
    ...
    async with db_session() as db:
        server = await ServerService.start_server(
            db, server_id, daemon_client=fake_client
        )
        await db.commit()
        assert server.crash_count == 0
...
@pytest.mark.asyncio
async def test_reconcile_restart_failure_leaves_crash_count_incremented_for_next_tick(
    db_session,
):
    ...
    async with db_session() as db:
        ...
        assert server.crash_count == 1
        assert (
            server.is_suspended is False
        )  # restart failing isn't itself grounds for suspension yet


@pytest.mark.asyncio
async def test_reconcile_fleet_skips_suspended_servers_and_returns_only_crashed_ids(
    db_session,
):
