...
async def test_logs_search_returns_matching_entries(client, db_session):
    ...
    response = await client.get(
        "/api/v1/logs", params={"query": "disk"}, headers=ADMIN_HEADERS
    )
...
async def test_security_events_lists_recorded_events(client, db_session):
    async with db_session() as db:
        db.add(
            SecurityEvent(event_type="auth_failure", source_ip="1.2.3.4", detail="{}")
        )
        await db.commit()
...
async def test_security_events_filters_by_event_type(client, db_session):
    async with db_session() as db:
        ...
        db.add(
            SecurityEvent(
                event_type="rate_limit_violation", source_ip="1.2.3.4", detail="{}"
            )
        )
        await db.commit()
    ...
    response = await client.get(
        ...
        params={"event_type": "rate_limit_violation"},
