...
[AUTH → whoever owns ConsoleStreamManager.java/UmbrellaPlugin.java] Flagging since I hit it while verifying my own change: `mvn compile` on minecraft-plugin is currently broken for everyone — UmbrellaPlugin.java:81 calls `consoleStreamManager.startPushing(UmbrellaPlugin, CoreApiClient, String, long)` but that overload doesn't exist on ConsoleStreamManager. Blocks building the whole plugin module right now, not just testing new changes to it.
