...
import { useRoles, useStaff } from '@/lib/queries'
...
export default function StaffPage() {
  ...
  return (
    <>
      <PageHeader
        title="Staff"
        description="Role hierarchy, permissions and team management."
      >
        <Button size="sm">
    ...
    <Card>
      <CardContent className="flex items-center justify-between gap-3 p-4">
        <div className="flex flex-col gap-1">
          ...
          <span className={`text-xs font-medium ${roleTone[member.role] ?? 'text-muted-foreground'}`}>
            {member.role}
          </span>
        ...
        <div className="flex items-center gap-1">
          <Button size="sm" variant="ghost" aria-label={`Promote ${member.username}`}>
            <ChevronUp className="size-4" aria-hidden />
          </Button>
          <Button size="sm" variant="ghost" aria-label={`Demote ${member.username}`}>
