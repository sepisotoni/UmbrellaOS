...
export const SettingsView: React.FC = () => {
  ...
  const [activeSection, setActiveSection] = useState<'core' | 'ai' | 'discord' | 'anticheat' | 'storage' | 'preferences' | 'templates' | 'player-experience'>('core');
  ...
  // Player Experience State (P16E)
  const [greeterEnabled, setGreeterEnabled] = useState(true);
  const [greeterFirstJoinMsg, setGreeterFirstJoinMsg] = useState('Welcome to the server, $PLAYER! Join our Discord: $DISCORD_INVITE');
  const [greeterReturnMsg, setGreeterReturnMsg] = useState('Welcome back, $PLAYER!');
  const [chatResponderEnabled, setChatResponderEnabled] = useState(true);
  const [chatResponderKeywords, setChatResponderKeywords] = useState<string[]>(['how to join', 'whats the ip', "what's the ip", 'how do i rank up', 'discord link', 'how do i appeal', 'how do i verify', 'what are the rules']);
  const [chatResponderNewKeyword, setChatResponderNewKeyword] = useState('');
  const [chatResponderCooldown, setChatResponderCooldown] = useState(60);
  const [chatResponderReplyMethod, setChatResponderReplyMethod] = useState<'chat' | 'dm'>('chat');
  const [chatResponderStyle, setChatResponderStyle] = useState('friendly and brief, 1-2 sentences max');
  const [playerExpLoading, setPlayerExpLoading] = useState(false);
  ...
  const sections = [
    ...
    { id: 'player-experience', label: 'Player Experience', sublabel: 'Greeter, AI chat responder', icon: Bot, badge: 'P16E' },
  ] as const;
  ...
  return (
    <div className="space-y-6 pb-12 font-sans">
      ...
      <div className="flex flex-col lg:flex-row gap-5 items-start w-full">
        ...
        {activeSection === 'player-experience' && (
          <div className="space-y-4">
            {/* Greeter */}
            <div className="rounded-xl border border-slate-800 bg-[#0c1017] p-5 space-y-5 font-mono text-xs">
              <div className="flex items-center gap-2 border-b border-slate-800 pb-3">
                <Bot className="h-4 w-4 text-cyan-400" />
                <span className="text-white font-bold">Greeter</span>
              </div>

              <div className="flex items-center justify-between">
                <div>
                  <p className="text-slate-200 font-semibold">Enable Greeter</p>
                  <p className="text-[10px] text-slate-500 mt-0.5">Send welcome messages to players on join</p>
                </div>
                <button
                  type="button"
                  onClick={() => {
                    const next = !greeterEnabled;
                    setGreeterEnabled(next);
                    api.updateSetting('greeter.enabled', String(next)).catch(() => {});
                  }}
                  className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors cursor-pointer ${greeterEnabled ? 'bg-cyan-600' : 'bg-slate-700'}`}
                >
                  <span className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${greeterEnabled ? 'translate-x-6' : 'translate-x-1'}`} />
                </button>
              </div>

              <div className="space-y-1.5">
                <label className="text-slate-200 font-semibold">First Join Message</label>
                <p className="text-[10px] text-slate-500">Variables: $PLAYER, $DISCORD_INVITE, $SERVER</p>
                <textarea
                  rows={2}
                  value={greeterFirstJoinMsg}
                  onChange={e => setGreeterFirstJoinMsg(e.target.value)}
                  className="w-full rounded-lg border border-slate-700 bg-slate-900 p-2.5 text-white text-xs focus:border-cyan-500 focus:outline-none resize-y font-mono"
                />
                <button
                  type="button"
                  onClick={() => api.updateSetting('greeter.first_join_message', greeterFirstJoinMsg).then(() => {}).catch(() => {})}
                  className="flex items-center gap-1.5 px-3 py-1 rounded-lg bg-cyan-900/50 hover:bg-cyan-800/60 border border-cyan-500/40 text-cyan-300 text-[11px] transition-colors cursor-pointer"
                >
              ...
              <div className="space-y-1.5">
                <label className="text-slate-200 font-semibold">Return Join Message</label>
                <p className="text-[10px] text-slate-500">Variables: $PLAYER, $SERVER</p>
                <textarea
                  rows={2}
                  value={greeterReturnMsg}
                  onChange={e => setGreeterReturnMsg(e.target.value)}
                  className="w-full rounded-lg border border-slate-700 bg-slate-900 p-2.5 text-white text-xs focus:border-cyan-500 focus:outline-none resize-y font-mono"
                />
                <button
                  type="button"
                  onClick={() => api.updateSetting('greeter.return_join_message', greeterReturnMsg).then(() => {}).catch(() => {})}
                  className="flex items-center gap-1.5 px-3 py-1 rounded-lg bg-cyan-900/50 hover:bg-cyan-800/60 border border-cyan-500/40 text-cyan-300 text-[11px] transition-colors cursor-pointer"
                >
                  <Save className="h-3 w-3" /><span>Save</span>
                </button>
            ...
            <div className="rounded-xl border border-slate-800 bg-[#0c1017] p-5 space-y-5 font-mono text-xs">
              <div className="flex items-center gap-2 border-b border-slate-800 pb-3">
                <MessageSquare className="h-4 w-4 text-cyan-400" />
                <span className="text-white font-bold">AI Chat Keyword Responder</span>
              </div>

              <div className="flex items-center justify-between">
                <div>
                  <p className="text-slate-200 font-semibold">Enable Chat Responder</p>
                  <p className="text-[10px] text-slate-500 mt-0.5">AI answers common questions detected in chat</p>
                </div>
                <button
                  type="button"
                  onClick={() => {
                    const next = !chatResponderEnabled;
                    setChatResponderEnabled(next);
                    api.updateSetting('chat_responder.enabled', String(next)).catch(() => {});
                  }}
                  className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors cursor-pointer ${chatResponderEnabled ? 'bg-cyan-600' : 'bg-slate-700'}`}
                >
                  <span className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${chatResponderEnabled ? 'translate-x-6' : 'translate-x-1'}`} />
                </button>
              </div>

              {/* Keyword tag input */}
              <div className="space-y-1.5">
                <label className="text-slate-200 font-semibold">Trigger Keywords</label>
                <p className="text-[10px] text-slate-500">Case-insensitive substring match. Press Enter to add.</p>
                <div className="flex flex-wrap gap-1.5 p-2 rounded-lg border border-slate-700 bg-slate-900 min-h-[36px]">
                  {chatResponderKeywords.map((kw, idx) => (
                    <span key={idx} className="flex items-center gap-1 px-2 py-0.5 rounded-md bg-cyan-900/60 border border-cyan-500/30 text-cyan-300 text-[11px]">
                      {kw}
                      <button
                        type="button"
                        onClick={() => {
                          const updated = chatResponderKeywords.filter((_, i) => i !== idx);
                          setChatResponderKeywords(updated);
                          api.updateSetting('chat_responder.keywords', JSON.stringify(updated)).catch(() => {});
                        }}
                ...
                <div className="flex gap-2">
                  <input
                    type="text"
                    value={chatResponderNewKeyword}
                    onChange={e => setChatResponderNewKeyword(e.target.value)}
                    onKeyDown={e => {
                      if (e.key === 'Enter' && chatResponderNewKeyword.trim()) {
                        e.preventDefault();
                        const kw = chatResponderNewKeyword.trim().toLowerCase();
                        if (!chatResponderKeywords.includes(kw)) {
                          const updated = [...chatResponderKeywords, kw];
                          setChatResponderKeywords(updated);
                          api.updateSetting('chat_responder.keywords', JSON.stringify(updated)).catch(() => {});
                        }
                        setChatResponderNewKeyword('');
                      }
                    }}
                    placeholder="Add keyword…"
                    className="flex-1 rounded-lg border border-slate-700 bg-slate-900 p-2 text-white text-xs focus:border-cyan-500 focus:outline-none font-mono"
                  />
                  <button
                    type="button"
                    onClick={() => {
                      const kw = chatResponderNewKeyword.trim().toLowerCase();
                      if (kw && !chatResponderKeywords.includes(kw)) {
                        const updated = [...chatResponderKeywords, kw];
                        setChatResponderKeywords(updated);
                        api.updateSetting('chat_responder.keywords', JSON.stringify(updated)).catch(() => {});
                      }
                    ...
                    className="px-3 py-1.5 rounded-lg bg-cyan-900/50 hover:bg-cyan-800/60 border border-cyan-500/40 text-cyan-300 text-[11px] cursor-pointer"
                  >Add</button>
                </div>
              </div>
              ...
              {/* Cooldown */}
              <div className="space-y-1.5">
                <label className="text-slate-200 font-semibold">Cooldown (seconds)</label>
                <p className="text-[10px] text-slate-500">How long before the same player can trigger another response</p>
                <div className="flex items-center gap-2">
                  <input
                    type="number"
                    min={0}
                    value={chatResponderCooldown}
                    onChange={e => setChatResponderCooldown(Number(e.target.value))}
                    className="w-24 rounded-lg border border-slate-700 bg-slate-900 p-2 text-white text-xs focus:border-cyan-500 focus:outline-none font-mono"
                  />
                  <button
                    type="button"
                    onClick={() => api.updateSetting('chat_responder.cooldown_seconds', String(chatResponderCooldown)).catch(() => {})}
                    className="flex items-center gap-1.5 px-3 py-1 rounded-lg bg-cyan-900/50 hover:bg-cyan-800/60 border border-cyan-500/40 text-cyan-300 text-[11px] cursor-pointer"
                  >
                    <Save className="h-3 w-3" /><span>Save</span>
                  </button>
                </div>
              ...
              {/* Reply method */}
              <div className="space-y-1.5">
                <label className="text-slate-200 font-semibold">Reply Method</label>
                <select
                  value={chatResponderReplyMethod}
                  onChange={e => {
                    const v = e.target.value as 'chat' | 'dm';
                    setChatResponderReplyMethod(v);
                    api.updateSetting('chat_responder.reply_method', v).catch(() => {});
                  }}
                  className="rounded-lg border border-slate-700 bg-slate-900 p-2 text-white text-xs focus:border-cyan-500 focus:outline-none font-mono cursor-pointer"
                >
                  <option value="chat">Chat (visible to all)</option>
                  <option value="dm">DM (private to player)</option>
                </select>
              </div>

              {/* Response style */}
              <div className="space-y-1.5">
                <label className="text-slate-200 font-semibold">Response Style Hint</label>
                <p className="text-[10px] text-slate-500">Tone/length instruction passed to the AI</p>
                <textarea
                  rows={2}
                  value={chatResponderStyle}
                  onChange={e => setChatResponderStyle(e.target.value)}
                  className="w-full rounded-lg border border-slate-700 bg-slate-900 p-2.5 text-white text-xs focus:border-cyan-500 focus:outline-none resize-y font-mono"
                />
                <button
                  type="button"
                  onClick={() => api.updateSetting('chat_responder.response_style', chatResponderStyle).catch(() => {})}
                  className="flex items-center gap-1.5 px-3 py-1 rounded-lg bg-cyan-900/50 hover:bg-cyan-800/60 border border-cyan-500/40 text-cyan-300 text-[11px] cursor-pointer"
                >
                  <Save className="h-3 w-3" /><span>Save</span>
                </button>
